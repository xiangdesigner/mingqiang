...
- Deployed to GitHub Pages (static export): `next.config.ts` has `output: "export"` and a custom image loader for basePath. The prefix variable is `NEXT_PUBLIC_BASE_PATH` on purpose: the loader also runs in the browser, and a non-public var is empty there (that bug shipped once: images 404'd after every client-side navigation). To verify basePath behaviour locally, build with `NEXT_PUBLIC_BASE_PATH=/mingqiang` and serve `out/` under that prefix (scratchpad `serve-out.mjs`), then client-navigate and check images. There is no `app/api/*` — GitHub Pages cannot run server code. Any new route must be a plain page (or a `force-static` metadata route). See README's Deployment/Inquiry form sections.
